...
5. **Eyeball the site:** "What changed today" panel (now has real movers), calibration page, **confirm fixture listings aren't doubled (triple_compare = 72 rows, enforced by FATAL assert)**, and a played match's page — note what it currently displays (result-vs-frozen-forecast display is NOT built yet; match pages don't read the wc_predictions.csv ledger). Observations feed Session 36.
