...
**Session 33 — Final data refresh (← NEXT)**
...
- **`generate_site.py` crashes on empty calibration bins once live results start (found Session 32 dry run):** `_cal_svg` does `round(b['mean_pred'] * 100)` per reliability bin; once `wc_predictions.csv` has live scored predictions but some bins are still empty (NaN `mean_pred`), `round(NaN)` raises `ValueError` and `update.py` aborts at the `generate_site` FATAL stage — the site stops updating on the first ~3 live match-days of the real tournament. Bins refill as matches accumulate so it self-clears mid-tournament, but the daily ritual breaks precisely at kickoff. Pre-tournament (backtest-seeded) all bins are populated so it doesn't fire today. Fix: guard empty/NaN bins in `_cal_svg`. Do before June 11.
---
