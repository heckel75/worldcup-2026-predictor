"""Build data/raw/r32_annex_c.csv from FIFA's Annex C of the 2026 WC regulations.

Source: https://en.wikipedia.org/wiki/2026_FIFA_World_Cup_knockout_stage
        (a transcription of Annex C of the FIFA tournament regulations)

For each of the 495 possible sets Q of 8 group letters whose third-placed teams
advance to the Round of 32, the table specifies which group's third-placed team
goes to each of the 8 R32 slots that face a group winner.

The 8 group winners that face a third-place team are A, B, D, E, G, I, K, L
(from the R32 schedule: matches 79, 85, 81, 74, 82, 77, 87, 80 respectively).

Output: data/raw/r32_annex_c.csv with one row per Q. Lookup key is the sorted
tuple of 8 qualifying group letters; value is the 8-element list of group
letters in SLOT_ORDER.

Validation enforced on every row before writing:
    1. 8 qualifying group letters, all in A-L, all distinct.
    2. 8 slot assignments, all distinct, all drawn from the qualifying set.
    3. Slot family membership: e.g. the team at slot 1A must come from
       {C, E, F, H, I} (FIFA's 5-group eligibility list for that slot).
    4. No same-group rematch: e.g. the team at slot 1A cannot come from A.

Run from project root:  python src/build_annex_c.py
"""
...
# --- Slot family constraints (from R32 fixture list, e.g. "1A vs 3CEFHI") ---
...
SLOT_ORDER: list[str] = ["1A", "1B", "1D", "1E", "1G", "1I", "1K", "1L"]

# --- Raw Annex C data (495 rows) ---
# Format per row: "N: q1; q2; ...; q8; 3a; 3b; ...; 3h"
# where q1..q8 are the qualifying group letters and 3a..3h are slot assignments
# in SLOT_ORDER. Lines copied verbatim from the Wikipedia transcription.
RAW_DATA = """
...
def parse_row(line: str) -> tuple[int, list[str], list[str]]:
    """Parse one Annex C line.

    Returns (row_number, qualifying_groups, slot_assignments).
    """
    # The first segment is "N: X" where N is the row number and X the first group.
    parts = [p.strip() for p in line.split(";")]
    n_str, first_q = parts[0].split(":", 1)
    parts[0] = first_q.strip()
    row_num = int(n_str.strip())
    qual = parts[:8]
    slots = [p.lstrip("3").strip() for p in parts[8:16]]
    return row_num, qual, slots


def validate_row(row_num: int, qual: list[str], slots: list[str]) -> None:
    """Enforce structural + FIFA constraints on a single row."""
    assert len(qual) == 8, f"row {row_num}: expected 8 qualifying groups, got {len(qual)}"
    assert len(set(qual)) == 8, f"row {row_num}: duplicate qualifying groups: {qual}"
    assert all(c in "ABCDEFGHIJKL" for c in qual), f"row {row_num}: invalid group letter in {qual}"
    assert len(slots) == 8, f"row {row_num}: expected 8 slot assignments, got {len(slots)}"
    assert set(slots) == set(qual), (
        f"row {row_num}: slot set {sorted(slots)} != qualifying set {sorted(qual)}"
    )
    for slot_id, third in zip(SLOT_ORDER, slots):
        assert third in SLOT_FAMILIES[slot_id], (
            f"row {row_num}: slot {slot_id} assigned 3{third}, "
            f"not in family {sorted(SLOT_FAMILIES[slot_id])}"
        )
        winner_group = slot_id[1]
        assert third != winner_group, (
            f"row {row_num}: slot {slot_id} would rematch group {winner_group} (3{third} vs 1{winner_group})"
        )
...
def main() -> None:
    ...
    for line in RAW_DATA.strip().splitlines():
        ...
        if not line:
            continue
        row_num, qual, slots = parse_row(line)
        validate_row(row_num, qual, slots)
        parsed.append((row_num, qual, slots))

    # Row count and row-number coverage.
    assert len(parsed) == 495, f"expected 495 rows, got {len(parsed)}"
    nums = [r[0] for r in parsed]
    assert nums == list(range(1, 496)), "row numbers are not 1..495 in order"

    # Every Q-set must be unique across the 495 rows.
    keys = {tuple(sorted(q)) for _, q, _ in parsed}
    assert len(keys) == 495, f"Q-sets are not all distinct: {len(keys)} unique"

    out_path = Path("data/raw/r32_annex_c.csv")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with out_path.open("w", newline="") as f:
        w = csv.writer(f)
        w.writerow(
            [f"Q{i}" for i in range(1, 9)]
            + [f"slot_{s}" for s in SLOT_ORDER]
        )
        for _, qual, slots in parsed:
            w.writerow(sorted(qual) + slots)

    print(f"Wrote {len(parsed)} rows to {out_path}")
    print(f"Q-sets unique: {len(keys) == 495}")
    print(f"All slot-family and no-rematch constraints satisfied.")
